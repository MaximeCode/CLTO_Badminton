...
const newsCards = [
  {
    ...
    title: 'Victoire éclatante de nos minimes',
    date: '3 mars 2026',
...
function NewsCard({ card }: { card: (typeof newsCards)[number] }) {
  return (
      ...
      <div className="group bg-white rounded-lg overflow-hidden shadow-md cursor-pointer transition-all duration-300">
        <div className="relative overflow-hidden aspect-[16/10]">
          ...
          <div className="absolute top-4 left-4">
            <span
              className={`text-white px-3 py-1 rounded-full text-sm font-medium ${card.categoryColor === 'secondary' ? 'bg-secondary' : 'bg-primary'
                }`}
            >
              {card.category}
            </span>
  ...
  return (
    <Section className="bg-white">
        ...
        <MobileCarousel
          ...
          renderItem={(card) => <NewsCard card={card} />}
          prevAriaLabel="Actualité précédente"
          nextAriaLabel="Actualité suivante"
          className="mb-12"
        />
